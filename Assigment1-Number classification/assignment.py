...
class Model:
    ...
    def __init__(self):
        ...
        self.batch_size = 10 # maximum accuracy (ish) = 10
    ...
    def back_propagation(self, inputs, outputs, labels):
        ...
        predicted_values = np.eye(self.num_classes)[prediction]
        
        expected_values = np.eye(self.num_classes)[np.ravel(labels)]
    
        y = expected_values - predicted_values
        weight_gradients = np.matmul(np.transpose(y), inputs) / self.num_classes
        bias_gradients = np.matmul(np.transpose(y), np.ones((len(inputs), 1))) / self.num_classes
    ...
    def accuracy(self, outputs, labels):
        ...
        alignments = (labels == outputs) * 1
        return np.sum(alignments) / len(outputs)
...
def test(model, test_inputs, test_labels):
    ...
    probability = model.call(test_inputs)
    accuracy = model.accuracy(np.argmax(probability, axis=1), test_labels)
    print(accuracy)
    return accuracy
...
def main(mnist_data_folder):
    ...
    train_inputs, train_labels = get_data('MNIST_data/train-images-idx3-ubyte.gz', 'MNIST_data/train-labels-idx1-ubyte.gz', trainingQuantityModel)
    
    test_inputs, test_labels = get_data('MNIST_data/t10k-images-idx3-ubyte.gz', 'MNIST_data/t10k-labels-idx1-ubyte.gz', testQuantity)    

    # TODO: Create Model
    NN_model = Model()

    # TODO: Train model by calling train() ONCE on all data
    train(NN_model, train_inputs, train_labels)

    # TODO: Test the accuracy by calling test() after running train()
    test(NN_model, test_inputs, test_labels)

    # TODO: Visualize the data by using visualize_results()
    visualize_results(test_inputs[0:10], NN_model.call(test_inputs[0:10]), test_labels[0:10])
